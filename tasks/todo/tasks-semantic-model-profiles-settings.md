...
- `packages/shared/paths.js` - Add helpers for instance settings paths.
...
- `packages/runner/src/run-log.js` - Existing trace collector; usage extraction can read the same Pi JSON events.
...
- [ ] 1.0 Define the instance settings contract
  - [ ] 1.1 Decide canonical file location, e.g. `agenthqHome()/settings.yml` or `agenthqHome()/settings/model-profiles.yml`.
  - [ ] 1.2 Define schema for `defaultModelProfile` and `modelProfiles` mapping profile keys to concrete Pi selectors.
  - [ ] 1.3 Decide pass-through behavior for concrete selectors that do not match a profile key.
  - [ ] 1.4 Define validation rules for profile keys, display labels, and model selector length/characters.
  - [ ] 1.5 Explicitly document that secrets/API keys remain in Pi config or env, never in settings.

- [ ] 2.0 Add shared settings path and loader support
  - [ ] 2.1 Add shared path helpers for settings file/dir under `agenthqHome()`.
  - [ ] 2.2 Add a small settings reader/parser with defaults when no settings file exists.
  - [ ] 2.3 Add model resolution helper: requested model/profile + settings => `{ requestedModel, resolvedModel, profileKey? }`.
  - [ ] 2.4 Ensure settings parsing errors are surfaced clearly in CLI and web UI.

- [ ] 3.0 Integrate semantic profiles into runner model resolution
  - [ ] 3.1 Resolve effective requested model from automation override, agent default, and instance default.
  - [ ] 3.2 Convert semantic profile keys to concrete Pi selectors before calling `pi --model`.
  - [ ] 3.3 Preserve pass-through for direct Pi model selectors according to the policy from 1.3.
  - [ ] 3.4 Include requested model, resolved model, and matched profile key in run trace metadata.
  - [ ] 3.5 Update DB schema/write/read logic if needed so run history can audit both human semantic name and concrete model.

- [ ] 3A.0 Add best-effort model usage logging
  - [ ] 3A.1 Add a runner helper that extracts usage from captured Pi JSON trace events and de-duplicates repeated response updates by `responseId` when present.
  - [ ] 3A.2 Normalize aggregate usage totals per run: input, output, reasoning if emitted, cache read/write, total tokens, reported cost/currency if present, provider, and Pi-reported model.
  - [ ] 3A.3 Persist usage on the `runs` row with nullable columns plus raw normalized `usage_json` for audit/debugging.
  - [ ] 3A.4 Keep usage extraction best-effort: missing/unknown usage must not fail the automation run.
  - [ ] 3A.5 Add a minimal usage summary reader grouped by resolved model/profile and/or Pi-reported model.
  - [ ] 3A.6 Show per-run usage in run detail and a simple aggregate usage view/table in the web app.
  - [ ] 3A.7 Defer provider pricing catalogs and per-turn usage tables unless Pi traces demonstrate that aggregates are insufficient.

- [ ] 4.0 Add minimal Settings UI
  - [ ] 4.1 Add `/settings` page with current settings file path, default model profile, and configured profiles.
  - [ ] 4.2 Add edit flow using either structured fields or a cautious raw YAML textarea.
  - [ ] 4.3 Validate settings before write and show parse/schema errors without corrupting the previous file.
  - [ ] 4.4 Add Settings navigation link.
  - [ ] 4.5 Update automation/agent forms to show available model profile keys as hints or selectable options if simple.

- [ ] 5.0 Update docs and examples without committing user-specific mappings
  - [ ] 5.1 Add docs explaining semantic model profiles and the Pi/provider boundary.
  - [ ] 5.2 Add a committed README/example that uses placeholder selectors only, not real user defaults.
  - [ ] 5.3 Update architecture/runtime flow to show model profile resolution before Pi invocation.
  - [ ] 5.4 Update deployment docs to include the instance settings file under `$AGENTHQ_HOME`.

- [ ] 6.0 Validate
  - [ ] 6.1 Add unit coverage for settings parsing, model resolution, and Pi usage extraction/de-duplication.
  - [ ] 6.2 Add web validation/smoke coverage for `/settings` and usage summary rendering without invoking Pi.
  - [ ] 6.3 Run `pnpm build`.
  - [ ] 6.4 Run `pnpm validate:web` and inspect Playwright output.
  - [ ] 6.5 Run `pnpm validate:backend` only if local Pi auth/provider availability is expected.
...
- Update `tasks/CHANGELOG.md` when complete.
