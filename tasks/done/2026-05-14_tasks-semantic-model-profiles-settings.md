...
Completed on 2026-05-14. Added instance-local semantic model profiles in `jumpyGoatHqHome()/settings.yml`, runner model resolution/audit metadata, best-effort Pi usage logging, `/settings` editing and usage summary UI, docs, and coverage. Backend smoke passed with Codex subscription after using provider-prefixed selectors (`fast` → `openai-codex/gpt-5.4-mini`).

## Goal

Add instance-level semantic model profiles so agents, automations, and future tasks can use names like `super-smart`, `fast`, or `tiny` while jumpyGoatHq resolves them to concrete Pi model selectors at runtime. Introduce a minimal Settings concept/UI for editing instance configuration without committing user-specific provider choices into the core app or public repo.

## Notes

- Model profile values are operator/instance state, not core product defaults. Store them under `jumpyGoatHqHome()` / `$JUMPYGOATHQ_HOME`, not in committed source files.
- Pi remains responsible for provider auth, API keys, custom providers, and real model availability. jumpyGoatHq only resolves semantic names to Pi `--model` selectors.
...
- Pi JSON traces already include best-effort `message.usage` details for some providers/models (`input`, `output`, `cacheRead`, `cacheWrite`, `totalTokens`, and sometimes `cost`). If Pi/provider exposes reasoning tokens, jumpyGoatHq should preserve them too, but not require them. jumpyGoatHq can normalize those events instead of estimating tokens itself.
...
- `packages/web/src/actions.ts` - Validate and persist settings edits safely under `jumpyGoatHqHome()`.
...
- `docs/DEPLOY.md` - Document `$JUMPYGOATHQ_HOME` settings file location and deployment behavior.
...
This is worth doing in core if it stays narrow: jumpyGoatHq runs Pi on behalf of scheduled/manual automations, already owns the run DB, and already captures Pi JSON traces. A user-facing "usage by model" view is operational run history, similar to status/duration/output, not provider configuration.
...
- Defer a `run_usage_events` child table until there is evidence that a single jumpyGoatHq run regularly uses multiple models/providers or needs per-turn billing audit.
...
- [x] 1.0 Define the instance settings contract
  - [x] 1.1 Decide canonical file location, e.g. `jumpyGoatHqHome()/settings.yml` or `jumpyGoatHqHome()/settings/model-profiles.yml`.
...
- [x] 2.0 Add shared settings path and loader support
  - [x] 2.1 Add shared path helpers for settings file/dir under `jumpyGoatHqHome()`.
...
- [x] 5.0 Update docs and examples without committing user-specific mappings
  ...
  - [x] 5.4 Update deployment docs to include the instance settings file under `$JUMPYGOATHQ_HOME`.
...
- Semantic model mappings are instance/operator configuration under `jumpyGoatHqHome()` / `$JUMPYGOATHQ_HOME`, not committed core app state.
- Pi owns provider auth and concrete model availability; jumpyGoatHq owns semantic policy and audit metadata.
- Usage logging is core runtime observability stored in the SQLite run DB, not a settings-file concern.
- Pi-emitted usage/cost is the only source of truth for token/cost reporting; jumpyGoatHq should not estimate tokens or maintain pricing tables in this feature.
