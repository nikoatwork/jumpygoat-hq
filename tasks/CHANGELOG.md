...
- feat: added v0 connector primitive with Resend email notification parsing/delivery, skill allowed intents, automation opt-in config, and run metadata; live email delivery awaits sender/recipient config.
...
- `tasks/todo/tasks-04-resend-notifications.md` — implementation mostly complete; build/web/backend validation passed, but real Resend email delivery smoke is blocked until sender/recipient config is provided.
